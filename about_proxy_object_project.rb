...
class Proxy
  ...
  def initialize(target_object)
    ...
    @messages = []
    @target_object_methods = Hash[target_object.methods.zip Array.new(target_object.methods.size,0)]
  end
  ...
  def called? method_name
    @target_object_methods[method_name] > 0
  end

  def number_of_times_called method_name
    @target_object_methods[method_name]
  end

  def method_missing(method_name, *args, &block)
    if @target_object_methods.keys.include?(method_name)
      @target_object_methods[method_name] += 1
